...
u_turn:		la	$a0, u_turn_msg		#gets users move
		...
		lw	$t0, col_bias		#convert column char into int
						#make case insensitive?
		sub	$s0, $s0, $t0		#use subu?
		
		li	$v0, 4			#check for valid input
		li	$t0, 15
		bltu	$s0, $t0, valid_col	#branch if column is valid (in range)
		la	$a0, oor_col
		syscall
		j	u_turn
valid_col:	li	$t0, 11
		bltu	$s1, $t0, valid_row	#branch if row is valid (in range)
		la	$a0, oor_row
		syscall
		j	u_turn
valid_row:	andi	$t0, $s0, 1		#get LSB of row and col to determine parity
		...
		syscall
		j	u_turn
valid_space:	sll	$t2, $s1, 4		#calculate displacement address of byte to update
		add	$t2, $t2, $s0
		lbu	$t3, board($t2)
		lbu	$t4, space
...
occ_space:	.asciiz "You can't place a line here because it already has a line! Try again.\n"
		.align	2
col_bias:	.ascii	"A"			#probably a better way to do this?
		.align	2
hor_line:	.ascii	"-"
		.align	2
ver_line:	.ascii	"|"
